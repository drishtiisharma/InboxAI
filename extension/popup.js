...
const themeToggle = document.getElementById("themeToggle"); // Added theme toggle
...
function updateUIForLoggedInUser() {
    ...
    // Hide login button in login view
...
function updateUIForLoggedOutUser() {
    ...
    // Show login button in login view
    ...
    const unlockSpeech = () => {
        if (!speechUnlocked && speechSynthesis) {
            try {
                ...
                // Speak greeting if not already spoken
                if (!greetingSpoken && chatView.style.display !== "none") {
...
function switchMode(mode) {
    ...
    if (meetingView) meetingView.style.display = "none";
    ...
    if (mode === "chat") {
        ...
        if (meetingView) meetingView.style.display = "block";
        resetMeetingMode();
        if (!currentUser) {
...
function showChatGreeting() {
    if (greetingSpoken || !chatMessages) return;
...
// Add event listeners to mode steps
...
function showStep(stepNumber) {
    console.log("Showing draft step:", stepNumber);
...
if (generateDraftsBtn) {
    generateDraftsBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/email/draft`, {
                method: "POST",
...
function displayDraftCards() {
    ...
    draftSuggestions.forEach((draft, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="draft-card-subject">Subject: ${draft.subject || "No subject"}</div>
            <div class="draft-card-body">${draft.body || "No content"}</div>
        `;
...
function selectDraft(index) {
    const cards = draftCards.querySelectorAll(".draft-card");
    cards.forEach(card => {
...
if (confirmSelectionBtn) {
    confirmSelectionBtn.addEventListener("click", () => {
        ...
        if (confirmSubject) confirmSubject.textContent = selectedDraft.subject || "No subject";
        if (confirmBody) confirmBody.textContent = selectedDraft.body || "No content";
    ...
    sendEmailBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/email/send`, {
                ...
                body: JSON.stringify({
                    to: recipient,
                    subject: selectedDraft.subject || "No subject",
            ...
            if (emailIntent) emailIntent.value = "";
            draftSuggestions = [];
            selectedDraftIndex = null;
            showStep(1);
...
if (generateMeetingBtn) {
    generateMeetingBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/meeting/create`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    title: title || "Meeting",
...
if (sendMeetingInviteBtn) {
    sendMeetingInviteBtn.addEventListener("click", async () => {
        if (!generatedMeetingData) {
            alert("No meeting data to send");
            return;
        }

        sendMeetingInviteBtn.disabled = true;
        sendMeetingInviteBtn.textContent = "Sending...";

        try {
            // Here you would send the actual invites
            ...
            sendMeetingInviteBtn.disabled = false;
            sendMeetingInviteBtn.textContent = "Send Invites";
        }
    });
}
...
function resetMeetingMode() {
    ...
    // Reset time type
    const timeType = document.getElementById("meetingTimeType");
    if (timeType) {
...
async function initializeApp() {
    console.log("Initializing InboxAI app...");
